...
final class BlackoutSeriesController extends AbstractController
{
    ...
    public function edit(Request $request, BlackoutSeries $blackoutSeries, EntityManagerInterface $entityManager): Response
    {
        ...
        // Pré-remplir les dates avec la première instance (si elle existe)
        $instance = $blackoutSeries->getInstances()->first();
        if ($instance) {
            $form->get('start')->setData($instance->getStartDate());
            $form->get('end')->setData($instance->getEndDate());
        }

        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            // Mettre à jour l'instance avec les nouvelles dates
            if ($instance) {
                $instance->setStartDate($form->get('start')->getData());
                $instance->setEndDate($form->get('end')->getData());
            } else {
                // Au cas où aucune instance n'existerait (sécurité)
                $newInstance = new BlackoutInstance();
                $newInstance->setStartDate($form->get('start')->getData());
                $newInstance->setEndDate($form->get('end')->getData());
                $newInstance->setSeries($blackoutSeries);
                $entityManager->persist($newInstance);
            }

            $blackoutSeries->setLastModified(new \DateTime());
            $entityManager->flush();
